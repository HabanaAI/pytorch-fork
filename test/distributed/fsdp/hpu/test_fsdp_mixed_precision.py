...
class TestFSDPMixedPrecision(FSDPTest):
    ...
    def _get_simple_model(self, param_dtype, *fsdp_args, **fsdp_kwargs):
        model = FSDP(
            LinearMixedPrecision(param_dtype).to(device_hpu), *fsdp_args, **fsdp_kwargs
        )
        return model
    ...
    def _run_test_mixed_precision_e2e(
        ...
        for model in fsdp_models:
            ...
            with patch_reduce_scatter(test_reduce_scatter, full_precision_param_dtype):
                ...
                for _ in range(3):
                    inp = torch.randn(
                        3, 10, device="hpu", dtype=full_precision_param_dtype
                    )
...
class TestFSDPMixedPrecisionSharded(TestFSDPMixedPrecision):
    ...
    def _test_mixed_precision_embedding_table(self, mp_config):
        ...
        with patch_reduce_scatter(test_reduce_scatter, param_dtype):
            ...
            for _ in range(6):
                ...
                loss = fsdp_model.module.get_loss(inp, output).to(device_hpu)
    ...
    def test_mixed_precision_resnet(self):
        ...
        for ResNet model.
        """
        resnet_model = torchvision.models.resnet50().to(device_hpu)
        ...
        inp = torch.ones(1, 3, 1000, 1000, device="hpu")
        mp_config = MixedPrecision(
            param_dtype=torch.float16,
            reduce_dtype=torch.float16,
            buffer_dtype=torch.float16,
    ...
    def test_mp_batchnorm(self, convert_sync_bn):
        ...
        net = BatchNormNet().to(device_hpu)
        if convert_sync_bn:
        ...
        inp = torch.randn((1, 2), device="hpu")
    ...
    def test_eval_root_cast_inputs(self):
        ...
        for use_full_prec_in_eval in [True, False]:
            ...
            m = MyModel().to(device_hpu)
            m.a = FSDP(m.a, mixed_precision=mp_config, **fsdp_kwargs)
            model = FSDP(m, mixed_precision=mp_config, **fsdp_kwargs)
            model.eval()
            inp = torch.randn(5, 5)
    ...
    def test_full_precision_in_eval(self):
        ...
        for (
            ...
            inp = module_accessor.get_input(torch.device("hpu"))
            output = model(*inp)
            loss = module_accessor.get_loss(inp, output).to(device_hpu)
    ...
    def test_full_precision_in_eval_comm(self):
        for (
            ...
            with patch_reduce_scatter(test_reduce_scatter, torch.float32):
                inp = model_accessor.get_input(torch.device("hpu"))
                output = model(*inp)
                loss = model_accessor.get_loss(inp, output).to(device_hpu)
    ...
    def test_submodules_with_external_inputs(self):
        ...
        class ToyModel(nn.Module):
            ...
            def forward(self, x: torch.Tensor) -> torch.Tensor:
                ...
                y = torch.ones(2, 100, device="hpu", dtype=torch.float32)
