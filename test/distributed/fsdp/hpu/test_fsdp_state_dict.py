...
class TestFSDPStateDict(FSDPTest):
    ...
    def _get_simple_nested_model(
        ...
        if wrap:
            lin1 = nn.Linear(10, 10, bias=False).to(device_hpu)
            lin2 = nn.Linear(10, 10, bias=False).to(device_hpu)
            if checkpoint_wrap:
            ...
            model = nn.Sequential(
                nn.Linear(10, 10, bias=False).to(device_hpu),
                nn.Linear(10, 10, bias=False).to(device_hpu),
            )
        return model

    def _get_simple_model(self, *fsdp_args, checkpoint_wrap=False, **fsdp_kwargs):
        lin = nn.Linear(10, 10, bias=False).to(device_hpu)
        if checkpoint_wrap:
    ...
    def _get_multibuffer_nested_model(
        ...
        if wrap:
            lin1 = nn.Linear(10, 10, bias=False).to(device_hpu)
            bn1 = nn.BatchNorm1d(10).to(device_hpu)
            lin2 = nn.Linear(10, 10, bias=False).to(device_hpu)
            if checkpoint_wrap:
                lin1 = checkpoint_wrapper(lin1)
            ...
            model = nn.Sequential(
                nn.Linear(10, 10, bias=False).to(device_hpu),
                nn.BatchNorm1d(10).to(device_hpu),
                nn.Linear(10, 10, bias=False).to(device_hpu),
            )
        return model
    ...
    def _get_non_fsdp_root_module(self, *fsdp_args, wrap=True, **fsdp_kwargs):
        class FSDPContainer(nn.Module):
            def __init__(self, fsdp_1, fsdp_2):
                super().__init__()
                self.non_fsdp_lin = nn.Linear(10, 10, bias=False).to(device_hpu)
                self.fsdp_1 = fsdp_1
                self.fsdp_2 = fsdp_2
    ...
    def test_basic_save_and_load_state_dict(
        ...
        for model_call in [
            ...
            inp = torch.randn((3, 10), device=device_hpu)
            if fp16:
                inp = inp.half()
            ...
            if not cpu_offload.offload_params:
                model_new = model_new.to(device_hpu)
            if fp16:
                model_new.half()
            # Run a forward/backward to compute gradients to test the case
            # where there are gradients populated
    ...
    def test_save_and_load_after_forward_state_dict(
        ...
        for _ in range(6):
            inp = torch.randn(1, 10, device=ht.hpu.current_device())
            output = model(*inp)
            loss = output.sum()
    ...
    def _initialize_model(
        ...
        model = Model(wrap_fsdp, register_buffers=register_buffers).to(device_hpu)
        if wrap_fsdp:
    ...
    def _dist_train(
        ...
        in_data = torch.rand(64, 4, requires_grad=True, device=torch.device("hpu"))
        for _ in range(3):
            out = model(in_data)
            out.sum().backward()
            optim.step()
        ...
        if wrap_fsdp:
            blank_model = FSDP(Model(True).to(device_hpu))
    ...
    def test_state_dict_load_into_local_module(
        ...
        if not fsdp_root:
            in_data = torch.randn(
                1, 10, requires_grad=True, device=torch.device("hpu")
            )
        else:
            in_data = torch.rand(64, 4, requires_grad=True, device=torch.device("hpu"))
        for _ in range(3):
    ...
    def test_wrong_state_dict_config(self):
        model = FSDP(Model(wrap_fsdp=True).to(device_hpu))
    ...
    def test_state_dict_with_ignored_modules(
        ...
        # the model
        nonwrapped_model = Model(wrap_fsdp=False, register_buffers=True).to(device_hpu)
    ...
    def test_local_state_dict_with_empty_ranks(self):
        ...
        model = FSDP(Model().to(device_hpu))
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            with torch.no_grad():
                with FSDP.summon_full_params(model):
                    ...
                    model.my_parameter.copy_(torch.full((1,), 1.75).to(device_hpu))
    ...
    def test_torch_save_load(self):
        model = Model(wrap_fsdp=True).to(device_hpu)
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            state_dict = model.state_dict()
    ...
    def test_shared_module_and_shared_parameter(self):
        ...
        model = FSDP(TestDummyModel().to(device_hpu))
